...
export async function planFromRequirements(): Promise<void> {
  const requirements = await vscode.window.showInputBox({
    title: 'SDD: Plan Specs from Requirements',
    prompt: 'Enter your requirements (what do you want to build?)',
  ...
  await vscode.window.withProgress(
    ...
    async (progress, token) => {
      ...
      const planPromise = orchestrator.plan({ requirements, existingSpecIds, projectPrefix });
      ...
      let result;
      try {
        result = await Promise.race([planPromise, cancelPromise]);
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        if (/cancelled/i.test(msg)) {
          vscode.window.showInformationMessage('Planning cancelled.');
        } else {
          vscode.window.showErrorMessage(`Planning failed: ${msg}`);
        }
        return;
      }

      if (!result.success) {
        vscode.window.showErrorMessage(`Planning failed: ${result.error}`);
        return;
      }
      ...
      vscode.window.showInformationMessage(
        `Planning complete: ${result.specs.length} spec(s) generated.`
      );
    }
  );
}
